...
void main(){
  ...
  vec3 fNorm = uvNormalMap( t_normal , vPos , vUv * 20. , vNorm , 2.1 , 1.7 );
  ...
  col *= vec3(210.,180.,140.)/255.; //texture2D( t_audio , vec2( fr , 0.)).xyz * fr;
  float alpha =  .1;
  if( res.y > -.5 ){
    ...
    col += vec3(210.,180./2.,140./2.)/255.  *  texture2D( t_matcap , semLookup( refr , n , modelViewMatrix , normalMatrix ) ).xyz;

    alpha = 1.;

    }else{
      col += vec3(0.,180.,140.)/255.  *  texture2D( t_matcap , semLookup( refr , n , modelViewMatrix , normalMatrix ) ).xyz;
    }
